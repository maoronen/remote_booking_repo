...
class Hotel:
    ...
    'hotel_rating': float 0-10. The rating number the hotel received
    'score_title': str. The rating title of the hotel
    'total_reviews': int. Number of reviews the hotel received
    'price': int. The price of the hotel for the entire requested period
    'location': str. Area / city or island where the hotel is located
    'meals': str. Which meals (if any) are included in the hotel price
    'room_type': str. The type of room priced at the hotel price
    'bed_type': str. Bed type/s in the hotel room
    'hotel_image': str. URL for the hotel image or hotel area"""
    ...
    def get_hotels_as_dict(self):
        """returns the hotel attributes as a dictionary
        ...
        with open("hotels_details.csv", "w", encoding='utf-8', newline="") as booking_file:
            ...
            for link in all_urls_list:
                ...
                for item in soup.select(cfg.HOTEL_BLOCK):
                    ...
                    writer.writerow({cfg.HOTEL_NAME: hotel_name,
                                     cfg.HOTEL_RATING: hotel_rating,
                                     cfg.SCORE_TITLE: score_title,
                                     cfg.NUMBER_OF_REVIEWS: total_reviews,
                                     cfg.PRICE: price,
                                     cfg.LOCATION: location,
                                     cfg.MEALS: meals,
                                     cfg.ROOM_TYPE: room_type,
                                     cfg.BED_TYPE: bed_type,
                                     cfg.HOTEL_IMAGE: hotel_image})
